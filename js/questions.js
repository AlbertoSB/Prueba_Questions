...
function gestionarXml(dadesXml){
 ...
 var opcionesSelect = [];
 var nopt = xmlDoc.getElementById("q_001").getElementsByTagName('option').length;
  for (i = 0; i < nopt; i++) { 
    opcionesSelect[i] = xmlDoc.getElementById("q_001").getElementsByTagName('option')[i].innerHTML;
 }
 ...
 numeroSecreto=parseInt(xmlDoc.getElementsByTagName("answer")[4].innerHTML);

 //CHECKBOX
 //Recuperamos el título y las opciones, guardamos las respuestas correctas
 var tituloCheckbox = xmlDoc.getElementsByTagName("title")[8].innerHTML;
 var opcionesCheckbox = [];
 var nopt = xmlDoc.getElementById("q_009").getElementsByTagName('option').length;
 for (i = 0; i < nopt; i++) { 
    opcionesCheckbox[i]=xmlDoc.getElementById("q_009").getElementsByTagName('option')[i].innerHTML;
 }  
 ponerDatosCheckboxHtml(tituloCheckbox,opcionesCheckbox);
 var nres = xmlDoc.getElementById("q_009").getElementsByTagName('answer').length;
 for (i = 0; i < nres; i++) { 
  respuestasCheckbox[i]=xmlDoc.getElementById("q_009").getElementsByTagName("answer")[i].innerHTML;
 }
}
